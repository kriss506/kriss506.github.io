...
function updateOutput() {
    ...
    for (let i = 0; i < t.length; i++) {
        ...
        dynamicCard.classList.add("dynamicCard", element.status);
        if (element.id == 10) { debugger; }
        dynamicCard.innerHTML = element.id + ": " + element.name + ", " + element.status;
        output.appendChild(dynamicCard);
    }
    //output.innerHTML += st;
}
...
function updateOutput2(tasks) {
    ...
    for (let i = 0; i < t.length; i++) {
        ...
        dynamicCard.classList.add("dynamicCard", element.status);
        dynamicCard.innerHTML = element.id + ": " + element.name + ", " + element.status;
        output2.appendChild(dynamicCard);
    }
}
...
function resetChecks() {
    ...
    PROGRESS = taskManager.getTasksWithStatus('INPROGRESS');
...
function init() {
    ...
    button03.addEventListener('click', () => {
        taskManager.deleteTask({ id: 10, name: 'Go fishing', description: 'Go fishing', assignedTo: 'Sid Smith', dueDate: new Date(), status: 'TODO' });
        updateOutput()
    });
    ...
    button07 = document.getElementById("button07");
    button07.addEventListener('click', () => { resetChecks(); updateOutput2(PROGRESS) });
    //updateOutput();
}

let button100 = null;
dge(button100, "button100");

function dge(button, buttonId) {
...
function assignElements(b, bi) {
    for (let i = 0; i < b.length; i++) {
        b[i] = document.getElementById(bi);
